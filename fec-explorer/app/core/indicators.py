...
def calculate_indicators(rows: list) -> list:
    ...
    for siret, vals in sorted(acc.items()):
        ...
        if vals["fournisseurs"] > 0:
            liquidite_generale = round((vals["clients"] + vals["stocks"] + vals["tresorerie"]) / vals["fournisseurs"], 2)
        elif vals["charges"] > 0:
            liquidite_generale = round(vals["tresorerie"] / (vals["charges"] / 12), 2)
        else:
            liquidite_generale = 0
        delai_client        = round((vals["clients"]      / vals["ca"])                  * 360, 2) if vals["ca"]                  > 0 else 0
        frs_abs       = abs(vals["fournisseurs"])
        achats_externes = (
        ...
        if frs_abs == 0 or vals["ca"] == 0:
            delai_fournisseur = 0
        elif achats_externes > 0 and frs_abs / vals["ca"] >= 0.3:
            delai_fournisseur = round((frs_abs / achats_externes) * 360, 2)
        else:
            delai_fournisseur = round((frs_abs / vals["ca"]) * 360, 2)
